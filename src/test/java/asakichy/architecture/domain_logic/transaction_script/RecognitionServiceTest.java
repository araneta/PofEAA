...
public class RecognitionServiceTest {
	...
	public void ある時点での収益認識() throws Exception {
		RecognitionService recognitionService = new RecognitionService(new Gateway(conn));

		// 商品「ワープロ」の契約
		recognitionService.calculateRevenueRecognitions(0);
		// 契約日に全て計上
		assertThat(recognitionService.recognizedRevenue(0, signedDate), is(100L));

		// 商品「データベース」の契約
		recognitionService.calculateRevenueRecognitions(1);
		// 契約時は1/3
		assertThat(recognitionService.recognizedRevenue(1, signedDate), is(33L));
		// 60日後に1/3
		assertThat(recognitionService.recognizedRevenue(1, addDays(signedDate, 60)), is(66L));
		// 90日後に残り
		assertThat(recognitionService.recognizedRevenue(1, addDays(signedDate, 90)), is(100L));

		// 商品「表計算」の契約
		recognitionService.calculateRevenueRecognitions(2);
		// 契約時は1/3
		assertThat(recognitionService.recognizedRevenue(2, signedDate), is(33L));
		// 30日後に1/3
		assertThat(recognitionService.recognizedRevenue(2, addDays(signedDate, 30)), is(66L));
		// 60日後に残り
		assertThat(recognitionService.recognizedRevenue(2, addDays(signedDate, 60)), is(100L));
	}
