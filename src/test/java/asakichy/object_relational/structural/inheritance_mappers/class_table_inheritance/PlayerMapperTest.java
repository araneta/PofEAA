...
public class PlayerMapperTest {
	...
	public void データ削除() throws Exception {
		...
		rs = stmt.executeQuery("SELECT count(*) FROM footballers");
		rs.next();
		count = rs.getInt(1);
		assertThat(count, is(0));
	...
	public void 準備() throws SQLException {
		...
		stmt.execute("CREATE TABLE players ( id BIGINT PRIMARY KEY, name VARCHAR(32), type VARCHAR(32) )");

		stmt.execute("DROP TABLE IF EXISTS footballers");
		stmt.execute("CREATE TABLE footballers ( id BIGINT PRIMARY KEY, club VARCHAR(32) )");

		stmt.execute("DROP TABLE IF EXISTS cricketers");
		stmt.execute("CREATE TABLE cricketers ( id BIGINT PRIMARY KEY, batting_average BIGINT )");

		stmt.execute("DROP TABLE IF EXISTS bowlers");
		stmt.execute("CREATE TABLE bowlers ( id BIGINT PRIMARY KEY, bowling_average BIGINT )");

		stmt.execute("INSERT INTO players VALUES(0,'山田太郎','FOOTBALL')");
		stmt.execute("INSERT INTO players VALUES(1,'山田花子','CRICKET')");
		stmt.execute("INSERT INTO players VALUES(2,'山田次郎','BOWLING')");

		stmt.execute("INSERT INTO footballers VALUES(0,'インテル')");
		stmt.execute("INSERT INTO cricketers VALUES(1,500)");
		stmt.execute("INSERT INTO cricketers VALUES(2,300)");
		stmt.execute("INSERT INTO bowlers VALUES(2,200)");
	}
